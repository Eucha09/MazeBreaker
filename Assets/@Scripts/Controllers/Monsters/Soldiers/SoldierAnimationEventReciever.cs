...
public class SoldierAnimationEventReciever : MonsterAnimationEventReciever
{
    ...
    public void SlashAttackEffect()
    {
        ...
        GameObject a = Managers.Resource.Instantiate("Effects/Soldier/SlashAttack", _monster.transform.position, _monster.transform.rotation);
    ...
    public void PoisonProjectileAttackEffect()
    {
        ...
        Debug.Log("이펙트 호출~~");
        GameObject a = Managers.Resource.Instantiate("Effects/Soldier/RangerAttack", new Vector3(_monster.transform.position.x, _monster.transform.position.y, _monster.transform.position.z), Quaternion.LookRotation(PlayerPos - _monster.transform.position));
        a.GetComponentInChildren<ProjectileDamageCollider>().Init(_monster);
